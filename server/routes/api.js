...
router.get("/GetAsutpResForPowerConsumption", (req, res, next) => {
  request(
    `http://${config.recalculationServerHost}:8081/GetAsutpResForPowerConsumption`,
    { json: true },
    (err, resp, body) => {
      if (err) return next(err);
...
router.get("/paramValues", (req, res, next) => {
  ...
  request(
    `http://${config.recalculationServerHost}:8081/GetHistoryOfParamValues?ParamName=${paramName}&FromDT=${req.query.fromDT}&ToDT=${req.query.toDT}`,
    { json: true },
    (err, resp, body) => {
      if (err) return next(err);
      res.status(200).json(body);
...
router.get("/getCollisions", (req, res, next) => {
  request(
    `http://${config.recalculationServerHost}:8081/GetRecalculationCollisions`,
    { json: true },
    (err, resp, body) => {
      if (err) return next(err);
      res.status(200).json(body);
...
try {
  ...
  // Send the file
    const response = await fetch(`http://${config.recalculationServerHost}:8081/UploadSapMetersFile`, {
...
try {
  ...
  // Send the file
    const response = await fetch(`http://${config.recalculationServerHost}:8081/UploadSignalsFile`, {
    // const response = await fetch('http://localhost:8081/UploadSapMetersFile', {
    method: "POST",
    body: form,
    headers: headers
