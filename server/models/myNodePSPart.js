...
class MyNodePSPart extends MyNode {
  ...
  makeChains() {
    ...
    for (let i = 0; i < this.sec2secConnectors.length; i += 1) {
      ...
      if (
        sec2secConnector.fromSection !== null &&
        sec2secConnector.toSection !== null
      ) {
        if (
          sec2secConnector.getSwitchedOn() &&
          MyChains.HoldersCouldBeConnected(
            sec2secConnector.fromSection.chain.holders.concat(
              sec2secConnector.toSection.chain.holders
            )
          )
        ) {
          // if (sec2secConnector.switchedOn) {
          sec2secConnector.fromSection.chain.connectedElements.push(
            sec2secConnector
          );
          sec2secConnector.fromSection.chain.join(
            sec2secConnector.toSection.chain
          );
        } else {
          sec2secConnector.fromSection.chain.disconnectedElements.push(
            sec2secConnector
          );
        }
      }
